...
RE_EMAIL_OBFUSCATED = re.compile(
    r"""
    (?P<user>[a-zA-Z0-9._%+-]{1,64})
    \s*(?:\(|\[)?\s*(?:at|@)\s*(?:\)|\])?\s*
    (?P<domain>[a-zA-Z0-9.-]{1,255})
    \s*(?:\(|\[)?\s*(?:dot|\.)\s*(?:\)|\])?\s*
    (?P<tld>[a-zA-Z]{2,24})
    """,
...
def normalize_email(user: str, domain: str, tld: str) -> str:
    return f"{user.strip().lower()}@{domain.strip().lower()}.{tld.strip().lower()}"
...
def crawl(seeds: List[str], max_pages: int, max_depth: int) -> None:
    ...
    while queue and page_count < max_pages:
        ...
        # ===== [ADD] email extraction (strict + obfuscated) =====
        emails_ex = extract_emails(text)

        emails_strict_n = len(emails_ex["strict"])
        emails_obf_n = len(emails_ex["obfuscated"])
        emails_total = emails_strict_n + emails_obf_n
    ...
    if fp in seen_fps:
    ...
    elif verdict.score >= 10 and emails_total >= 1 and should_save(verdict):
        row = {
            "run_id": RUN_ID,
            "ts": utc_now_iso(),
            "url": url,
            ...
            "counts": {
                ...
                "emails_strict": emails_strict_n,
                "emails_obfuscated": emails_obf_n,
            ...
            "fingerprint": fp,
            # "emails_hashes": {
            #     "strict": emails_ex["strict_hashes"],
            #     "obfuscated": emails_ex["obfuscated_hashes"],
            # },
        }
        add_result(row)
        seen_fps.add(fp)
        ...
        log_event({"stage": "fp_skip", "url": url, "fp": fp, "score": verdict.score, "type": verdict.leak_type, "signals": verdict.signals})
